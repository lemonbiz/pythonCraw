...
def get_shop_information(url_shop):
	...
	try:
		...
		open_time = None
	return 1, phone, open_time, is_wifi
...
def get_shop_comment(uuid, id):
	...
	url_comment = r'http://i.meituan.com/xiuxianyule/api/getCommentList?poiId='+str(id)+'&offset='+str(num)+'&pageSize=10&sortType=1&mode=0&starRange=10%2C20%2C30%2C40%2C50&tag='
	time.sleep(random.randint(1,5) + random.uniform(10, 20)/4)
	html_comment = down(url_comment)
	#解析第一页的标签
	try:
		review_labels_list = []
		review_labels_list = json.loads(html_comment)['data']['commentTagDTOList']
	except Exception as e:
	...
	try:
		comments_list = []
		comments_list = json.loads(html_comment)['data']['commentDTOList']
	except Exception as e:
	...
	while True:
		num += 10
		url_comment = r'http://i.meituan.com/xiuxianyule/api/getCommentList?poiId='+str(id)+'&offset='+str(num)+'&pageSize=10&sortType=1&mode=0&starRange=10%2C20%2C30%2C40%2C50&tag=%E5%85%A8%E9%83%A8'
...
def down_comment(id):
	url_source = 'http://www.meituan.com/xiuxianyule/'
	data = {}
	data['SHOP_ID'] = id
